...
*I didn't get a warning until I removed `warning = FALSE` from the global options in the setup chunk* The last name in row 49 gets discarded from the dataset. 

```{r}
# Your code here: 
sw_split_name <- starwars %>% # Start with starwars
  separate(col = name, # Separate the name column...
           into = c("name_1", "name_2"), # Into 2 columns, name_1 and name_2
           sep = "-") # Separated by the first "-"
...
sw_skin_sep <- starwars %>% # Start from starwars data
  separate(col = skin_color, # Separate the `skin_color` column...
           into = c("skin_1","skin_2"), # into 2 columns...
           sep = ",", # Creating the separation at the first comma (,)
           extra = "merge", # If there are additional commas, merge all into second column
           remove = FALSE) # And do not remove the original column
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create a new column called `model_class`
        c(model, class), # That combines model and class columns
...
class(some_dates_converted) # Check class of vector 
```
...
gasol_hookshots <- lakers %>% # Using lakers create a new data set...
  filter(player == "Pau Gasol", type == "hook") %>% # Including only Pau Gasol's hook shots
  mutate(date_new = ymd(date)) %>% # Create a new column with dates converted to "Date" class
  mutate(month_new = month(date_new)) # Create a new column containing the month

gasol_hookshots_month <- gasol_hookshots %>% # Create a summary table...
  group_by(month_new) %>% # of Pau Gasol's hook shot count grouped by month 
  summarize(hook = n())
